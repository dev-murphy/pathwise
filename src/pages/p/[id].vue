...
const route = useRoute<'/p/[id]'>()
const flowId = String(route.params.id)
const { getFlow, incrementAnalytic } = useFlowStore()

const flow = getFlow(flowId)
const isPublished = flow?.status === 'published'

const nodes: Node[] = isPublished ? (flow?.nodes ?? []) : []
const edges: Edge[] = isPublished ? (flow?.edges ?? []) : []

const VISIT_KEY = `pathwise_p_visited_${flowId}`
const COMPLETE_KEY = `pathwise_p_completed_${flowId}`

onMounted(() => {
  if (!flow || !isPublished) return
  try {
    if (!sessionStorage.getItem(VISIT_KEY)) {
      incrementAnalytic(flowId, 'visits')
...
function markCompleted() {
  ...
  try {
    if (!sessionStorage.getItem(COMPLETE_KEY)) {
      incrementAnalytic(flowId, 'completions')
...
<template>
  <div class="min-h-screen bg-bg flex flex-col">
    <!-- Minimal public-facing nav: just brand, no builder affordances -->
    ...
    <div v-if="!flow" class="grow flex flex-col items-center justify-center gap-y-4 text-center px-6">
      ...
      <p class="text-muted font-mono text-sm">Form not found.</p>
    </div>

    <!-- Draft gate -->
    <div v-else-if="!isPublished" class="grow flex flex-col items-center justify-center gap-y-4 text-center px-6">
      <AlertCircle class="w-12 h-12 text-muted" />
      <p class="text-text font-mono text-sm">This form isn't published yet.</p>
      <p class="text-muted font-mono text-xs">The owner must publish it before it can be filled out.</p>
    </div>

    <!-- Empty (published but no fields) -->
    <div v-else-if="!startNode" class="grow flex flex-col items-center justify-center gap-y-4 text-center px-6">
      <AlertCircle class="w-12 h-12 text-muted" />
      <p class="text-muted font-mono text-sm">This form has no fields yet.</p>
    </div>
    ...
    <div v-else-if="completed" class="grow flex flex-col items-center justify-center gap-y-6 text-center px-6">
      ...
      <div class="flex flex-col gap-y-1">
        ...
        <p class="text-muted font-mono text-sm">Thanks for filling out this form.</p>
      </div>
      <button
